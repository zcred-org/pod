...
export class ZCredStoreApi {
  ...
  public readonly zkpResultCache;

  constructor(didStore: typeof DidStore) {
    this.context = {
      axios: axios.create({ baseURL: config.zCredStoreOrigin.href }),
      didStore,
    };
    this.context.axios.interceptors.request.use(async (config) => {
      if (/credential|zkp-result-cache/.test(config.url?.toLowerCase() ?? '')) {
    ...
    this.zkpResultCache = new ZkpResultCacheApi(this.context);
  }

  #getToken(): Promise<string> {
    this.#tokenPromise ??= (async () => {
      const did = this.context.didStore.$did.peek();
...
export const zCredStore = new ZCredStoreApi(DidStore);
